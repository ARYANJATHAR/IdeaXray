...
import { AnalysisStarter } from "@/components/analysis/analysis-starter";
...
export default async function AnalyzePage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  if (id) return <><SiteHeader analysis /><main id="main-content" className="analysis-main page-width"><AnalysisController key={id} id={id} /></main><SiteFooter /></>;

  const parsed = analysisInputSchema.safeParse({
    idea: initialIdea,
    region: region.success ? region.data : "worldwide",
  });
  if (parsed.success && parsed.data.idea.trim()) {
    return (
      <>
        <SiteHeader analysis />
        <main id="main-content" className="analysis-main page-width">
          <AnalysisStarter input={parsed.data} />
        </main>
        <SiteFooter />
      </>
    );
  ...
  return (
    <>
      <SiteHeader analysis />
      <main id="main-content" className="analysis-main page-width">
        <Link href="/" className="back-link"><IconArrowLeft size={17} aria-hidden="true" />Back to the clearing</Link>
